...
def busca_profundidade(grafo, inicio, fim):
    caminhos = []
    busca_profundidade_rec(grafo, inicio, fim, caminhos)
    caminhos.sort(key=lambda t: t[1])
    return caminhos


def busca_profundidade_rec(grafo, inicio, fim, caminhos, comprimento=0, caminho=[]):
    ...
    for vizinho in grafo.neighbors(inicio):
        if vizinho not in caminho:
            busca_profundidade_rec(grafo, vizinho, fim, caminhos, 
                                    comprimento + comp_rua, caminho + [inicio])
    return



##Ignora caminhos piores q o melhor já encontrado
def busca_profundidade_melhorado(grafo, inicio, fim):
    caminhos = []
    busca_profundidade_rec_melhorado(grafo, inicio, fim, caminhos)
    caminhos.sort(key=lambda t: t[1])
    return caminhos

def busca_profundidade_rec_melhorado(grafo, inicio, fim, caminhos, menor_comprimento=1_000_000, comprimento=0, caminho=[]):
    comp_rua = grafo.get_edge_data(caminho[-1], inicio, 0)['length'] if (caminho) else 0
    ...
    for vizinho in grafo.neighbors(inicio):
        if vizinho not in caminho:
            menor_comprimento = busca_profundidade_rec_melhorado(grafo, vizinho, fim, caminhos, menor_comprimento,
                                    comprimento + comp_rua, caminho + [inicio])
    return menor_comprimento
...
t0 = time.time()
caminhos = busca_profundidade_melhorado(grafo, inicio, fim)
dt = time.time()-t0
